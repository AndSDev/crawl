...
static bool _reroll_random(newgame_def& ng)
{
    ...
    tilep_race_default(ng.species, 1, &doll);
    tilep_job_default(ng.job, &doll);
#ifdef USE_TILE_LOCAL
    ...
    for (int j = 0; j < TILEP_PART_MAX; j++)
    {
        if (doll.parts[j] != 0 && doll.parts[j] != TILEP_SHOW_EQUIP)
        {
            tiles.json_open_array();
            tiles.json_write_int(doll.parts[j]);
            tiles.json_write_int(TILE_Y);
                       ...
                       UINewGameMenu* ng_menu, menu_letter &letter)
{
    ...
    for (job_type &job : jobs)
    {
        ...
        ng_menu->_add_group_item(
            ...
            get_job_name(job),
#ifdef USE_TILE
            tile_def(tileidx_player_job(job), TEX_GUI),
#endif
            is_active_item,
            pos
        );
...
#ifdef USE_TILE
            tile_def(tileidx_player_species(this_species), TEX_PLAYER),
#endif
            is_active_item,
            pos
        );
                                   ...
                                   shared_ptr<OuterMenu>& sub_items)
{
    ...
    for (unsigned int i = 0; i < weapons.size(); ++i)
    {
        ...
        string text = make_stringf(" %c - %s   (%+d apt)", letter,
